 ...
 * by doing a JOIN to include data from other tables. We start with SELECT FROM
 * left_table, and we can JOIN right_table.
 *
 * There are several types of joins. We will be looking at INNER JOIN here.
 * INNER JOIN is when we query two or more tables on some criteria, and only see
 * results where there are matching rows in all tables.
 *
 ...
 * ON keywords. We join one table to another based on some condition that
 * matches rows from both tables together. In the above example we are matching
 * rows from table_left which have the same value in column 1 as those in
 * table_right column 3.
 *
 ...
 * included in the results. This is because there is no matching record in the
 * opposite table for either of those records. (There is no art teacher, and
 * there are no students taking biology.)
 *
...
public class InnerJoinActivity {
    ...
    public Set<Pair<Integer, String>> problem1() {
        /**
         * Problem 1: Write a query that will return the id, and name of each of Ms.
         * Lovelace's students. Notice that Ms. Lovelace teaches two classes, but which
         * classes she teaches aren't known from the data in the student table. This
         * means that you will need a way to combine the data from the two tables (inner
         * join).
         *
         * You will need to simultaneously filter those results WHERE class.teacher_name
         ...
         *
         *
         * Note: There should not be a wild card (*) in your statement. You will need to
         * specify the column in your statement by writing table.column, because the
         * column names may be ambiguous between class and student.
         */
