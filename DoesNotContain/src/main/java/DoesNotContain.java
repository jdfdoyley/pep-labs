
public class DoesNotContain {
    /**
     * The inverse of the contains value challenge: check if the array does *not*
     * contain the value. Note that this is not as simple as flipping the return
     * true/false from the 'Contains' challenge.
     * You should think critically about the logic that you write within the for
     * loop.
     *
     * For a trickier problem like this, you could even try explaining your thought
     * process of working through this problem with pencil and paper in plain
     * English before attempting it in code.
     *
     * @param arr    the array we will be manipulating.
     * @param target the value we are searching for.
     * @return true if arr does not contain target. false otherwise.
     */
    public boolean arrayDoesNotContain(int[] arr, int target) {
        for (int i : arr) {
            if (i == target) {
                return false;
            }
        }
        return true;
    }
}
