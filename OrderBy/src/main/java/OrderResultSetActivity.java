 ...
 * ordered. Physically, that information is serialized in some order, but it is
 * not necessarily the order you might expect, and the order is subject to
 * change. When querying for information we usually want to indicate the order
 * for our results.
 ...
 * BY is in ascending order. Ascending order places lesser values before greater
 * values. Descending order is the opposite, greatest values first.
 *
 * For example, one might add the ORDER BY clause after the WHERE clause of a
 * query to order employees from highest to lowest salary:
 *
...
public class OrderResultSetActivity {
    ...
    public List<Character> problem1() {
         ...
         * Make sure the results are in ascending order by last name, and first name as
         * a tie-breaker.
         */
